...
exports.sendSticker = (from, filename, bull) => {
        leo.sendMessage(from, filename, MessageType.sticker, {quoted: bull})
}
...
exports.FakeStatusImgForwarded = (from, image, caption) => {leo.sendMessage(from, image, MessageType.image, { quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "mimetype": "image/jpeg", "caption": 'Thebull', "jpegThumbnail": fs.readFileSync(`./media/imagen/fake.jpg`)} } }, caption: caption, contextInfo: {"forwardingScore": 999, "isForwarded": true} })
}
...
exports.sendPdf = async(from, buffer, title = "bull.pdf") => {
    await bull.sendMessage(from, buffer, MessageType.document, { mimetype: Mimetype.pdf, title: title })
}
...
exports.sendFakeThumb = async(from, buffer, caption = "") => {
    ...
    await bull.sendMessage(from, buffer, MessageType.image, options)
}

exports.downloadMedia = async(media) => {
    const filePath = await bull.downloadAndSaveMediaMessage(media, `./temp/${getRandomExt()}`)
