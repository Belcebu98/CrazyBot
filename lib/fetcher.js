...
exports.modStick = (media, client, bull, from) => {
    out = getRandom('.webp')
    try {
        console.log(media)
        spawn('webpmux', ['-set','exif', './sticker/data.exif', media, '-o', out])
        .on('exit', () => {
            client.sendMessage(from, fs.readFileSync(out), 'stickerMessage', {quoted: bull})
            fs.unlinkSync(out)
            fs.unlinkSync(media)
        })
    } catch (e) {
        console.log(e)
        client.sendMessage(from, 'Se produjo un error', 'conversation', { quoted: bull })
        fs.unlinkSync(media)
    }
}